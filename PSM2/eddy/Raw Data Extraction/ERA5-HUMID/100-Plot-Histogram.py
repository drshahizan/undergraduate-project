...
#location=['Museum-Padi','SK-Wan-Yahya','Pendang-Lake-Resort']
...
for locations in location:
    ...
    for x,y in zip(bulan,frame):
        ...
        plt.xticks(tahun[::2],rotation=90,ha='left',fontsize=5)
        ##plt.xticks(x, labels[::2], rotation='vertical')### lompat lompat
        
        plt.ylim(60,100)
        #plt.ylim(range(0,800,100))
        plt.yticks(np.arange(60,100,5),fontsize=8)
...
#######################################################        
        ...
        # plot title
        plt.title('Monthly Average Relative Humidity Trend Month '+str(x),fontsize=10)
        plt.suptitle('Monthly Average Relative Humidity Trend for '+locations+ '\n from 1940-2022',fontsize=14,y=0.94)
