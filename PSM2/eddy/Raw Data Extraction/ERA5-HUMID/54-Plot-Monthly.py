...
for files in folder:
    ...
    humid_rate = data[:, 2]
    humid_rate = [float(i) for i in humid_rate]
    
    print(max(humid_rate))
    print(humid_rate[10])
    print(lat[0])
    print(max(humid_rate))
    ...
    # Fill the 2D array with the humid rate data
    Z[lat_idx, lon_idx] = humid_rate
    print(Z.shape)
    
    # Setting up the plot for humidity
    ...
    cmap = cm.s3pcpn_l
    clevprecip = np.arange(60, 100, 0.2)
    ...
    cbar.set_label('Monthly Average Humidity (%)', fontsize=10)
    
    # Add title to the plot
    title1 = 'ERA5- Monthly Average Humidity for ' + nama_file
    plt.title(title1)
    
    # Adjust the naming convention for the saved images
    save_name = f'Humidity {nama_file}.png'
