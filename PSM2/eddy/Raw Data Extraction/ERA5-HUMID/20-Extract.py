...
for month in files_month:
    filename = f'Monthly-HUMID-{month}-1940-2024.nc'
    ...
    for index, date in enumerate(ptime):
        ...
        # Ensure humid data is reshaped to match the lat/lon grid
        if humid.ndim == 3:
            humid_data = humid[index, :, :]
        elif humid.ndim == 2:
            humid_data = humid
        else:
            raise ValueError("Unexpected humid data dimensions")
        
        lon_grid, lat_grid = np.meshgrid(lon, lat)
        
        # Prepare data for saving
        extracted_data = np.column_stack((lat_grid.ravel(), lon_grid.ravel(), humid_data.ravel()))
