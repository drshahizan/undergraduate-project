...
for files in folder:
    ...
    temp_rate = data[:, 2]
    temp_rate = [float(i) for i in temp_rate]
    ...
    # Fill the 2D array with the temp rate data
    Z[lat_idx, lon_idx] = temp_rate
    print(Z.shape)
    
    # Setting up the plot for temperature
    ...
    m = Basemap(projection='merc', llcrnrlat=0., llcrnrlon=95.,
                urcrnrlat=10., urcrnrlon=120., resolution='i')
    ...
    cmap = cm.s3pcpn_l
    clevprecip = np.arange(20, 35, 0.1)
    ...
    cbar.set_label('Monthly Average Temperature (Degree Celcius)', fontsize=10)
    
    # Add title to the plot
    title1 = 'ERA5- Monthly Average Temperature for ' + nama_file
    plt.title(title1)
    
    # Adjust the naming convention for the saved images
    save_name = f'Temperature {nama_file}.png'
