<template>
    <StudentLayout :page-title="'Rapor'">
        <div class="gap-[1.5rem] flex flex-1 justify-end items-end mx-12 mt-12">
            <Dropdown v-model="selectedCity" :options="cities" optionLabel="name" placeholder="Tahun Ajaran"
                class="flex-1 w-[2rem] md:w-12rem" />
            ...
            <Dropdown v-model="selectedCity" :options="cities" optionLabel="name" placeholder="Kelas"
                class="flex-1 w-[2rem] md:w-12rem" />
            <Button type="button" label="Unduh" severity="success" />

        </div>
        <div class="flex justify-center items-center mt-12">
            <img class="w-1/2" src="/images/rapor.png" alt="">
        </div>
    </StudentLayout>
</template>
