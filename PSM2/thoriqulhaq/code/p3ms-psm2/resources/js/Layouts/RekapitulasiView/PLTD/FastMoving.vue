    ...
    setup(props) {
        ...
        const getMaterial = () => {
            ...
            props.data?.detail?.selectedMaterial?.forEach((item) => {
                string += `<div class='flex justify-between items-center mr-[0.5rem] mb-[0.75rem]'><span>${formatString(item.material)} - ${item.status}</span> <span>(${item.quantity} Buah)</span></div>`;
            });
            return string;
        }
