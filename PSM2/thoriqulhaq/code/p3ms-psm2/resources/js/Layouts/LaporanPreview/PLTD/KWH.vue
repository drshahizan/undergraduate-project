    ...
    <table class="text-[11px] w-full text-center border-collapse border border-[#495157]">
        <thead>
            <tr>
                ...
                <th class="border border-[#495157]" :colspan="$page.props.user.engine_quantity">ENGINE MODULE (MESIN)</th>
                <th class="border border-[#495157]" :colspan="$page.props.user.engine_quantity">KWH METER PEMBANDING (MESIN)</th>
        ...
        <tbody v-if="datas?.length != 0">
            <template v-for="data in datas" :key="data">
                <tr>
                    ...
                    <template v-for="n in $page.props.user.engine_quantity" :key="n">
                        <td class="border border-[#495157]">{{data['engine_'+n+'_em']}}</td>
                    </template>
                    <template v-for="n in $page.props.user.engine_quantity" :key="n">
                        <td class="border border-[#495157]">{{data['engine_'+n+'_kmp']}}</td>
                    </template>
