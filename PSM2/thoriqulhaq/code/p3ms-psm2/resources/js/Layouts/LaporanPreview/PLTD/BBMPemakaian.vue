    ...
    <table class="text-[11px] w-full text-center border-collapse border border-[#495157]">
        <thead>
            <tr>
                ...
                <th class="border border-[#495157]" :colspan="$page.props.user.engine_quantity">TANGKI HARIAN (MESIN)</th>
                <th class="border border-[#495157]" :colspan="$page.props.user.engine_quantity">MODULE PCC 300 (MESIN)</th>
                <th class="border border-[#495157]" :colspan="$page.props.user.engine_quantity">FLOW METER (MESIN)</th>
                <th class="border border-[#495157]" rowspan="2">FLOW METER INDUK</th>
        ...
        <tbody v-if="datas?.length != 0">
            <template v-for="data in datas" :key="data">
                <tr>
                    ...
                    <template v-for="n in $page.props.user.engine_quantity" :key="n">
                        <td class="border border-[#495157]">{{data['engine_'+n+'_mp3']}}</td>
                    </template>
                    <template v-for="n in $page.props.user.engine_quantity" :key="n">
                        <td class="border border-[#495157]">{{data['engine_'+n+'_fm']}}</td>
                    </template>
                    <td class="border border-[#495157]">{{data.master_flow_meter}}</td>
        ...
        <tbody v-else>
            <tr>
                <td class="border border-[#495157]" :colspan="3+$page.props.user.engine_quantity*3">Tidak ada data</td>
            </tr>
        </tbody>
    </table>
</template>
