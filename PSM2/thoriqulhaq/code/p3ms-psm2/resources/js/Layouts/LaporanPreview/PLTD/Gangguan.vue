    ...
    <table class="text-[11px] w-full text-center border-collapse border border-[#495157]">
        <thead>
            <tr>
                ...
                <th class="border border-[#495157]">STATUS</th>
                <th class="border border-[#495157]">NOMOR LH5</th>
                <th class="border border-[#495157]">KETERANGAN</th>
        ...
        <tbody v-if="datas?.length != 0">
            <template v-for="data in datas" :key="data">
                <tr v-for="(item, index) in data.selectedMaterial" :key="item">
                    ...
                    <td class="border border-[#495157]" v-if="index == 0" :rowspan="data.selectedMaterial.length">{{data.unit}}</td>
                    <td class="border border-[#495157]" v-if="index == 0" :rowspan="data.selectedMaterial.length">{{data.status}}</td>
                    <td class="border border-[#495157]" v-if="index == 0" :rowspan="data.selectedMaterial.length">{{data.lh_five_no}}</td>
                    <td class="border border-[#495157]" v-if="index == 0" :rowspan="data.selectedMaterial.length">{{data.description}}</td>
        ...
        <tbody v-else>
            <tr>
                <td class="border border-[#495157]" :colspan="8">Tidak ada data</td>
            </tr>
        </tbody>
    </table>
</template>
