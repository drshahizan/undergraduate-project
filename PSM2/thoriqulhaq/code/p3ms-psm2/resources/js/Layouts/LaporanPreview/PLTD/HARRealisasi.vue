<template>
    <table class="text-[11px] w-full text-center border-collapse border border-[#495157]">
        ...
        <tbody v-if="datas?.length != 0">
            <template v-for="data in datas" :key="data">
                <tr v-for="(item, index) in data.selectedMaterial" :key="item">
                    ...
                    <td class="border border-[#495157]" v-if="index == 0" :rowspan="data.selectedMaterial.length">{{data.activity}}</td>
        ...
        <tbody v-else>
            <tr>
                <td class="border border-[#495157]" :colspan="6">Tidak ada data</td>
            </tr>
        </tbody>
    </table>
</template>
