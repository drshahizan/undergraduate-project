<template>
    <MainLayoutAdmin sideBar="3">
        <div class="flex justify-between items-center mb-[2rem]">
            ...
            <div>
                <Link href="/admin/kelola-master-data/material/input" class="py-[0.5rem] px-[1rem] flex justify-center items-center bg-[#14A3B8] rounded-[0.5rem] drop-shadow-[0px_12px_24px_rgba(0,0,0,0.04)]">
                    <p class="text-white text-[14px]">Tambah Material</p>
                </Link>
            </div>
        </div>
        <div class="bg-white flex-1 rounded-[2rem] p-[0.5rem]">
            <div class="card h-full">
                    <DataTable removableSort v-model:filters="filters" :value="data" paginator :rows="6" dataKey="id" :loading="loading" class="bg-white rounded-[1rem] flex flex-col h-full" headerClass="bg-white border-none" tableClass="border-none"
                        ...
                        <template #header>
                            <div class="flex justify-between items-center">
                                <div class="flex gap-[1.5rem]">
                                    ...
                                    <Dropdown v-model="selectedPowerPlant" :options="optionPowerPlant" optionLabel="name" placeholder="Pilih Lokasi"
                                    class="w-full md:w-14rem border-[1.5px] border-[#F8F8F9] bg-[#F8F8F9] rounded-[0.5rem]" :inputStyle="{ 'width': '100%' }" inputClass="p-inputtext-sm  h-[42px]"
                                    panelClass="text-[0.875rem]" />
                                </div>
                                <span class="p-input-icon-right">
                                    <InputText v-model="filters['global'].value" placeholder="Cari Material" class="p-inputtext-sm border-[1.5px] border-[#F8F8F9] bg-[#F8F8F9] rounded-[0.5rem] h-[42px]"/>
                        ...
                        <Column sortable field="description" header="Nama Material" style="min-width: 9rem">
                            <template #body="{ data }">
                                {{ data.description }}
                            </template>
                        </Column>
                        <Column sortable field="name" header="Pembangkit" style="min-width: 9rem">
                            <template #body="{ data }">
                                {{ data.name }}
                            </template>
                        </Column>
                        <Column sortable field="quantity" header="Jumlah" style="min-width: 9rem">
                            <template #body="{ data }">
                                {{ data.quantity }}
                            </template>
                        </Column>
                        <Column sortable field="created_at" header="Tanggal" style="min-width: 9rem">
                            <template #body="{ data }">
                        ...
                        <Column field="aksi" header="Aksi" style="width: 9rem">
                            <template #body="{ data }">
                                <div class="flex justify-center items-center gap-[1rem]">
                                    <Link :href="'/admin/kelola-master-data/material/edit/' + data.id">
                ...
                <div class="w-[30rem] h-[15rem] bg-white rounded-[1.5rem] drop-shadow-[0px_12px_24px_rgba(0,0,0,0.04)] flex flex-col">
                    ...
                    <div class="flex justify-end items-center px-[2rem] py-[1.25rem] gap-[1rem]">
                        ...
                        <Link :href="'/admin/kelola-master-data/material/delete/' + idHapusData" class="py-[0.5rem] px-[1rem] flex justify-center items-center bg-[#EF406C] rounded-[0.5rem] drop-shadow-[0px_12px_24px_rgba(0,0,0,0.04)]">
...
export default {
    ...
    setup(props) {
        ...
        const filters = ref({
            ...
            power_plant_id: { value: null, matchMode: FilterMatchMode.CONTAINS },
    ...
    watch: {
        selectedPowerPlantType(newValue) {
            ...
            this.optionPowerPlant = this.listPowerPlant.filter((item) => item.type == newValue.name);
        },
        selectedPowerPlant(newValue) {
            this.filters['power_plant_id'].value = newValue ? newValue.id : null;
        }
    },
}
</script>