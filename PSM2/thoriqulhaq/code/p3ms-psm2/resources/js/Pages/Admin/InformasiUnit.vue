    ...
    <MainLayoutAdmin sideBar="3">
        <div class="flex justify-between items-center mb-[2rem]">
            <div class="flex flex-col">
                ...
                <p class="text-[12px]">Data Unit</p>
            </div>
            <div>
                <Link href="/admin/kelola-master-data/unit/input" class="py-[0.5rem] px-[1rem] flex justify-center items-center bg-[#14A3B8] rounded-[0.5rem] drop-shadow-[0px_12px_24px_rgba(0,0,0,0.04)]">
                    <p class="text-white text-[14px]">Tambah Unit</p>
                </Link>
            </div>
        </div>
        <div class="bg-white flex-1 rounded-[2rem] p-[0.5rem] relative">
            <div class="card h-full">
                    <DataTable removableSort v-model:filters="filters" :value="data" paginator :rows="6" dataKey="id" :loading="loading" class="bg-white rounded-[1rem] flex flex-col h-full" headerClass="bg-white border-none" tableClass="border-none"
                        ...
                        <template #header>
                            <div class="flex justify-end">
                                <span class="p-input-icon-right">
                                    <InputText v-model="filters['global'].value" placeholder="Cari Unit" class="p-inputtext-sm border-[1.5px] border-[#F8F8F9] bg-[#F8F8F9] rounded-[0.5rem] h-[42px]"/>
                        ...
                        <Column sortable field="name" header="Nama Unit" style="min-width: 9rem">
                            <template #body="{ data }">
                                {{ data.name }}
                            </template>
                        </Column>
                        ...
                        <Column field="aksi" header="Aksi" style="width: 9rem">
                            <template #body="{ data }">
                                <div class="flex justify-center items-center gap-[1rem]">
                                    <Link :href="'/admin/kelola-master-data/unit/edit/' + data.id">
                ...
                <div class="w-[30rem] h-[15rem] bg-white rounded-[1.5rem] drop-shadow-[0px_12px_24px_rgba(0,0,0,0.04)] flex flex-col">
                    ...
                    <div class="flex justify-end items-center px-[2rem] py-[1.25rem] gap-[1rem]">
                        ...
                        <Link :href="'/admin/kelola-master-data/unit/delete/' + idHapusData" class="py-[0.5rem] px-[1rem] flex justify-center items-center bg-[#EF406C] rounded-[0.5rem] drop-shadow-[0px_12px_24px_rgba(0,0,0,0.04)]">
