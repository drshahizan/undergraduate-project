...
temperature_data = pd.read_csv("temperature_data_jb.csv")
wind_data = pd.read_csv("wind_data_jb.csv")
humidity_data = pd.read_csv("humidity_data_jb.csv")
rainfall_data = pd.read_csv("rainfall_data_jb.csv")

# rainfall_data = pd.read_csv("rainfall_data_melaka.csv")
# temperature_data = pd.read_csv("temperature_data_melaka.csv")
# humidity_data = pd.read_csv("humidity_data_melaka.csv")
# wind_data = pd.read_csv("wind_data_melaka.csv")

# Function to train models and make predictions for a given dataset
def train_and_predict(data, data_type, metric):
    ...
    ax.set_xlabel('Month')
    ax.set_ylabel(f'Predicted {data_type} ({metric})')
    ax.set_title(f'Predicted {data_type} ({metric}) in 2024')
    ax.set_xticks(index + bar_width / 2)
    ax.set_xticklabels(months, rotation=45)
    ax.legend()
    plt.show()

# Call the function for each type of data with the appropriate metric
train_and_predict(rainfall_data, "Rainfall", "mm")
train_and_predict(temperature_data, "Temperature", "°C")
train_and_predict(humidity_data, "Relative Humidity", "%")
train_and_predict(wind_data, "Wind Speed", "m/s")
